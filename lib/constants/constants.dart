class Constants {
  Constants._();

  static const String tag = 'FORGE';

  static const String defaultName = 'Forge Dancer';
  static const String defaultEmail = 'hello@forge.dance';
  static const String termOfService = 'https://forge.dance/terms';
  static const String privacyPolicy = 'https://forge.dance/privacy';
  static const String aboutUs = 'https://forge.dance/about';
  static const String rateUs = 'https://forge.dance';
  static const String facebookPage = 'https://forge.dance';
  // SharedPreferences key
  static const String themeModeKey = 'theme_mode_key';
  static const String profileKey = 'profile_key';
  static const String isLoginKey = 'is_login_key';
  static const String isExistAccountKey = 'is_exist_account_key';
}
