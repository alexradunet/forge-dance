...
`test/unit_test.dart` (validators + profile VM),
`test/authentication_test.dart` (stream-driven VM + fake auth repo),
`test/learn_test.dart` (progress VM + fake repo and catalog step fallback),
`test/workout_test.dart` (catalog rotation, session dedupe, workout VM),
`test/stats_test.dart` (XP, belts, streak rules), and
`test/app_redirect_test.dart` (pure-function matrix). Run with `flutter test`
— **codegen must have run first** (or just run `bash tool/checks.sh`, which
...
- Catalog rule changes: assert stable ids/coverage and update stats calibration
  tests when lesson XP totals change.
- Workout/session changes: cover deterministic `wodFor`, `WorkoutSession.docKey`
  dedupe, and the profile stats side effect.
