...
| `FgPhotoTile` / `FgPhotoTileLayout` | One native destination action per image card; two-up discovery becomes stacked at large text | Available in Widgetbook; not used on Home |
...
Home retains only the first **Continue Training** card, even when several modules are in progress. The oversized **Make it your own** block, duplicate utility shortcuts, and bottom information/development disclosures are removed. Home keeps its editorial hero, today's practice action, current lesson, progress and available recommendations—not a second navigation menu.

Learn stays in bottom navigation; Programmes is available from Learn, assessments from Profile (and the progress card), and the logbook from Workout. The motion lab is no longer linked from Home, including debug builds. Photo explanations remain in Workout and Vocabulary; decorative photo labels are retained so the images are not presented as movement demonstrations.

Rendered Home contracts at320/1040px with2× text assert the removed controls/disclosures are absent, multiple in-progress modules yield one continue card, and the retained lesson/practice/progress actions open their actual destinations. Normal/2× emulator screenshots were inspected and the runtime error check was empty. All409 core tests, browser integration and release-web quality passed (55/100/81/100;8.11MiB;zero console errors/failed requests). Evidence is under ignored `build/home-focus/`; the disconnected physical phone has not received this update. Emulator text size was restored to normal on Home.

## Motion lab: executable interaction experiment

The prototype page remains in source with direct widget/controller tests; it has no Home entry or product-navigation link, even in debug builds. Exercise it with `test/motion_lab_test.dart` and the motion-lab cases in `test/feature_surface_contract_test.dart`. No storage, lesson assessment, practice time, or rewards are affected.
...
| Home `/main/home` | **Refined**: poster, one current lesson, progress/recommendations, loading/error; no utility/development footer | Central Home absence/core-route contracts; current normal/2× captures in `build/home-focus/` |
...
| Debug motion lab (direct tests, no product-navigation entry) | **Regression**: warning and synthetic projected rig remain prototype-only; removed from Home | Central + deterministic controller tests; parent final normal/2×/controls/expanded limitations screenshots; no production 3D claim |
...
## Consolidated daily Workout session (2026-09-23)

This supersedes the earlier first-round/pending-result description for **Daily Practice only**. Programme and Vocabulary still launch the standalone player and own its returned pending record; legacy fitness circuits retain their own flow and confirmation copy.

- **Start workout** snapshots the current daily plan into one immersive root route. The overview retains preferences, full adaptations and related-lesson routes, but no longer offers per-round shortcuts. Round status, Previous/Next and horizontal swipes share one guarded transition; a swipe is not permission to bypass an unfinished round.
- The transient `WorkoutSession` controller retains clocks and draft choices while the shared `PracticePlayerPage` mounts only the displayed round. Count-in/pauses do not count toward the target. Previous and backgrounding pause playback; returning never auto-starts. Media initializes on demand. Session navigation details are separately actionable in the accessibility tree.
- Daily rounds require measured target completion before Save & next. The existing practice VM saves each exact frozen record before advancing; failed saves retain a visible Retry and require explicit confirmation to abandon. Confirmation/exit pauses and preserves drafts on Cancel. Saved/failed-save review exposes readable, immutable reflection and disables media mutation before repository IO. Standalone early Save is unchanged.
- Skip explicitly discards unsaved practice/reflection and never fabricates a record. Summary distinguishes completed and skipped rounds; an all-skipped session claims no completed practice. A failed acknowledgement may already have committed, so abandon copy warns that the log may contain that save and never deletes it. No durable schema, backend, dependency, platform setting or public diagnostic route was added.
- `test/workout_session_test.dart` covers transition gates, retained choices, count-in/target, frozen identity, serialized retry (including actual repository commit-before-error), confirmation/background invalidation and the 24-hour record bound. `workout_session_surface_contracts.dart` is part of the real feature-surface suite: light host, 1×/2×, 320/1040 widths and compact height, root confirmations, accessible navigation, slider/vertical gestures, save failure/retry, readonly media/notes, summary/exit, and the actual overview/root-route/VM shell guard. Existing standalone, Programme and Vocabulary contracts remain green.
- Final bounded gates: `tool/checks.sh` passed with clean analyzer and **428 tests**; browser integration passed; release Wasm/JS-fallback quality passed at **55 / 100 / 81 / 100**, **8.11 MiB**, zero console errors/failed requests. Logs are `build/consolidated-workout/{checks,integration,web-quality}-final.log`. The non-fatal Lantern `NO_LCP` diagnostic remains; these startup scores are not native-media benchmarks.
- Parent-operated live checkpoints covered normal/2× Android navigation, paused Cancel/Skip/Previous/Leave, compact-height confirmation and all-skipped summary. An isolated test-only browser entry (`test/fixtures/workout_session_live.dart`, optional `?scale=2`) exercised in-memory commit-before-error/Retry, completed review, full reflection semantics, expanded navigation and mixed summary under a light host. It writes no real practice records. Android captures include `second-large-*.png`, `second-compact-exit.png`, `second-summary.png` and `seventh-exit.png` under `build/consolidated-workout/`; browser screenshot/AX inspection was parent-owned. This is not physical screen-reader, native audio/video/import or iOS validation.
